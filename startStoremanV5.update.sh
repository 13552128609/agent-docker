...
# filebeat config and log path
filebeatLog=$workPath'/filebeat/logs'
filebeatConfig=$workPath'/filebeat/cfg'
filebeatData=$workPath'/filebeat/data'
filebeatImage="docker.elastic.co/beats/filebeat:8.11.0"
mkdir -p "$filebeatConfig" "$filebeatLog" "$filebeatData"
sudo chown -R "$USER" "$workPath/filebeat"
...
# add filebeat begin
# build filebeat config
cat <<EOF > $filebeatConfig/filebeat.yml
filebeat.inputs:
- type: filestream 
  id: mpc-log-$( [ "$isTestnet" == true ] && echo "debug" || echo "main" )
  enabled: true
  paths:
  ...
  fields:
    service: schnorrmpc
    type: "$( [ "$isTestnet" == true ] && echo "debug" || echo "main" )"
